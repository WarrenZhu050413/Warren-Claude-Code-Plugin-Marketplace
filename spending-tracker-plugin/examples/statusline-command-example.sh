#!/bin/bash
# Example Statusline with Spending Tracker Integration
# This is a complete working example based on Warren's existing statusline
# Copy this to ~/.claude/my-statusline.sh and customize as needed
...
if [ -n "$cost_raw" ] && [ "$cost_raw" != "null" ]; then
    ...
    if (($(echo "$cost_dollars > 0" | bc -l))); then
        ...
        # This uses session-based delta tracking (only adds new spending)
